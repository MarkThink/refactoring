...
class NoCaptcha
{
    ...
    //构造方法初始化
    public function __construct($em,$redis,$font,$output)
    {
        $this->font = $font;//注意字体路径要写对，否则显示不了图片
        $this->output = $output;
        $this->getIdiom($em,$redis);
    }

    /*
     * 获取三条成语+唯一标识汉字
     */
    public function getIdiom($em,$redis)
    {
        //如果成语键不存在，则生成成语数据缓存
        $is_idiom = $redis->exists('idiom');
        if(!$is_idiom){
            $idiom = $em->getRepository('Model:TrjCyDict')->getCyName();
            //缓存所有-小于或等于七个汉字的成语数据
            foreach($idiom as $value){
                $redis->hset('idiom',$value['id'],$value['chengyu']);
            }
        }
        //每次随机获取三条成语数据
